...
export class JiraDataMapper {
    ...
    static mapChangelog(jiraChangelog: any, issueId: string): DatabaseChangelog {
        return {
            ...
            author_name: jiraChangelog.author?.name || jiraChangelog.author?.display_name || jiraChangelog.authorId || 'unknown',
            author_email: jiraChangelog.author?.email || jiraChangelog.author?.emailAddress || '',
            items: JSON.stringify(jiraChangelog.items || [])
            // created_at, updated_at will be auto-generated by database
        };
    }
    ...
    static extractStatusChangeChangelogs(jiraIssue: any): DatabaseChangelog[] {
        ...
        for (const changelog of rawChangelogs) {
            ...
            if (changelog.items && Array.isArray(changelog.items)) {
                const hasStatusChange = changelog.items.some((item: any) => item.field === 'status');

                if (hasStatusChange) {
                    // Create a changelog record for this status change
                    changelogs.push(this.mapChangelog(changelog, issueId));
                }
            }
        }
